...
def training_loop(
    ...
    while True:
        ...
        if (sample_ticks is not None) and (done or cur_tick % sample_ticks == 0) and cur_tick > resume_tick and dist.get_rank() == 0:
            ...
            save_image_grid(images[:36], os.path.join(run_dir, f'1_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
            ...
            images = torch.cat(images).numpy()
            save_image_grid(images[:36], os.path.join(run_dir, f'2_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
