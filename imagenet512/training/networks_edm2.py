    ...
    def __init__(self,
        ...
        **block_kwargs,                     # Arguments for Block.
        ...
        for level, channels in enumerate(cblock):
            ...
            enc_block_kwargs = block_kwargs.copy()
            if dropout_res and res > dropout_res:
            ...
            if level == 0:
                ...
                self.enc[f'{res}x{res}_down'] = Block(cout, cout, cemb, flavor='enc', resample_mode='down', **enc_block_kwargs)
            for idx in range(num_blocks):
                cin = cout
                cout = channels
                self.enc[f'{res}x{res}_block{idx}'] = Block(cin, cout, cemb, flavor='enc', attention=(res in attn_resolutions), **enc_block_kwargs)
        ...
        for level, channels in reversed(list(enumerate(cblock))):
            res = img_resolution >> level
            dec_block_kwargs = block_kwargs.copy()
            if dropout_res and res > dropout_res:
                dec_block_kwargs['dropout'] = 0.0
            if level == len(cblock) - 1:
                self.dec[f'{res}x{res}_in0'] = Block(cout, cout, cemb, flavor='dec', attention=True, **dec_block_kwargs)
                self.dec[f'{res}x{res}_in1'] = Block(cout, cout, cemb, flavor='dec', **dec_block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = Block(cout, cout, cemb, flavor='dec', resample_mode='up', **dec_block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = channels
                self.dec[f'{res}x{res}_block{idx}'] = Block(cin, cout, cemb, flavor='dec', attention=(res in attn_resolutions), **dec_block_kwargs)
        ...
        for name, block in self.enc.items():
            if hasattr(block, 'dropout'):
                dist.print0(block)
                dist.print0(name, block.dropout)
                dist.print0('\n')
