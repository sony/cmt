...
def learning_rate_schedule_quad(cur_nimg, batch_size, ref_lr=100e-4, ref_batches=70e3, rampup_kimg=None):
    lr = ref_lr
    if ref_batches > 0:
...
def learning_rate_schedule_lin(ref_lr, cur_nimg, total_kimg):
    lr = ref_lr
    cur_kimg = cur_nimg / 1000
    lr *= (1 - cur_kimg / total_kimg)
    return lr
...
def training_loop(
    ...
    ref_network_kwargs  = {},       # Options for model and preconditioning.
    loss_kwargs         = {},       # Options for loss function.
    lr_kwargs           = {},       # Options for learning rate.
    optimizer_kwargs    = {},       # Options for optimizer.
    sch                 = "Linear", # Options for scheduler
    ...
    resume_ckpt         = None,     # Start from the given network snapshot, None = random initialization.
    ...
    interface_kwargs = dict(img_resolution=64, img_channels=4, label_dim=1000)
    net = dnnlib.util.construct_class_by_name(**network_kwargs, **interface_kwargs) # subclass of torch.nn.Module
    net.train().requires_grad_(True).to(device)
    net_ref = dnnlib.util.construct_class_by_name(**ref_network_kwargs, **interface_kwargs) # subclass of torch.nn.Module
    net_ref.eval().requires_grad_(False).to(device)
    ...
    if resume_pkl is not None:
        dist.print0(f'Loading encoder weights from "{resume_pkl}"...')
        if dist.get_rank() != 0:
        ...
        misc.copy_params_and_buffers(src_module=data['ema'], dst_module=net_ref, require_all=False)
    ...
    if resume_ckpt is not None:
        dist.print0(f'Loading network weights from "{resume_ckpt}"...')
        if dist.get_rank() != 0:
            torch.distributed.barrier() # rank 0 goes first
        with dnnlib.util.open_url(resume_ckpt, verbose=(dist.get_rank() == 0)) as f:
            data = pickle.load(f)
        if dist.get_rank() == 0:
            torch.distributed.barrier() # other ranks follow
        misc.copy_params_and_buffers(src_module=data['ema'], dst_module=net, require_all=True)
    ...
    grid_z = torch.randn([256, 4, 64, 64], device=device)
    grid_z = grid_z.split(batch_gpu)

    indices = np.random.randint(0, 1000, size=256)
    labels = torch.from_numpy(np.eye(1000)[indices]).float().to(device)
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                loss = loss_fn(net=ddp, net_ref=net_ref, g_net=gnet, images=images, labels=labels)
                training_stats.report('Loss/loss', loss)
                loss.mul(loss_scaling).mean().backward()
                
        # Update weights.
        if sch == "Quad":
            lr = learning_rate_schedule_quad(cur_nimg=cur_nimg, batch_size=batch_size, rampup_kimg=lr_rampup_kimg, **lr_kwargs)
        elif sch == "Linear":
            lr = learning_rate_schedule_lin(ref_lr=optimizer_kwargs['lr'], cur_nimg=cur_nimg, total_kimg=total_kimg)
        elif sch == "Constant":
        ...
        fields += [f"lr {training_stats.report0('LR/lr', lr):<.6e}"]
        ...
        # Sample Img
        if (sample_ticks is not None) and (done or cur_tick % sample_ticks == 0) and dist.get_rank() == 0: #and cur_tick > resume_tick 
            dist.print0('Exporting sample images...')
            images = [encoder.decode(generator_fn(ema, z, c)).cpu() for z, c in zip(grid_z, grid_c)]
            images = torch.cat(images).numpy()
            save_image_grid(images[:100], os.path.join(run_dir, f'1_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
        
            few_step_fn = functools.partial(generator_fn, mid_t=mid_t)
            images = [encoder.decode(few_step_fn(ema, z, c)).cpu() for z, c in zip(grid_z, grid_c)]
            images = torch.cat(images).numpy()
            save_image_grid(images[:100], os.path.join(run_dir, f'2_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
        
            del images

        # Save full dump of the training state.
        if (state_dump_ticks is not None) and (done or cur_tick % state_dump_ticks == 0) and cur_tick != 0 and dist.get_rank() == 0:
            dist.print0(f'Save the latest dump at {cur_tick:06d} img...')
            torch.save(dict(net=net, ema=ema, optimizer_state=optimizer.state_dict()), os.path.join(run_dir, f'training-state-latest.pt'))
        ...
        if (ckpt_ticks is not None) and (done or cur_tick % ckpt_ticks == 0) and cur_tick != 0:
            ...
            data = dict(ema=copy.deepcopy(ema).cpu().eval().requires_grad_(False))
            fname = f'network-snapshot-{cur_tick:06d}.pkl'
        ...
        if (eval_ticks is not None) and (done or cur_tick % eval_ticks == 0) and cur_tick > resume_tick:
            dist.print0('Evaluating EMA models...')
            one_step_results = metric_main.calc_metric(metric='fid50k_full',
                    generator_fn=generator_fn, G=ema, G_kwargs={}, encoder=encoder,
                    dataset_kwargs=dataset_kwargs, num_gpus=dist.get_world_size(), rank=dist.get_rank(), device=device)
            if dist.get_rank() == 0:
                metric_main.report_metric(one_step_results, run_dir=run_dir, snapshot_pkl=f'network-snapshot-{cur_tick:06d}-{cur_nimg//1000:07d}.pkl')
        
            few_step_fn = functools.partial(generator_fn, mid_t=mid_t)
            few_step_results = metric_main.calc_metric(metric='two_step_fid50k_full', 
                    generator_fn=few_step_fn, G=ema, G_kwargs={}, encoder=encoder,
                    dataset_kwargs=dataset_kwargs, num_gpus=dist.get_world_size(), rank=dist.get_rank(), device=device)
            if dist.get_rank() == 0:
                metric_main.report_metric(few_step_results, run_dir=run_dir, snapshot_pkl=f'network-snapshot-{cur_tick:06d}-{cur_nimg//1000:07d}.pkl')   
