...
#----------------------------------------------------------------------------
# A linear lr decay for resuming

def learning_rate_schedule(ref_lr, cur_nimg, total_kimg):
    lr = ref_lr
...
def training_loop(
    ...
    ref_network_kwargs  = {},       # Options for reference model and preconditioning.
    loss_kwargs         = {},       # Options for loss function.
    lr_kwargs           = {},       # Options for learning rate.
    optimizer_kwargs    = {},       # Options for optimizer.
    sch                 = 0,        # Options for scheduler
    encoder_kwargs      = {},       # Stable Diffusion Encoder
    ...
    if resume_pkl is not None:
        dist.print0(f'Loading network weights and encoder from "{resume_pkl}"...')
        if dist.get_rank() != 0:
    ...
    if resume_state_dump:
        ...
        misc.copy_params_and_buffers(src_module=data['net_ref'], dst_module=net_ref, require_all=True)
    ...
    grid_z = torch.randn([64, 4, 64, 64], device=device)
    grid_z = grid_z.split(batch_gpu)

    indices = np.random.randint(0, 1000, size=64)
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                images = torch.zeros([batch_gpu, net.img_channels, net.img_resolution, net.img_resolution], device=device)
                indices = np.random.randint(0, 1000, size=batch_gpu)
                labels = torch.from_numpy(np.eye(1000)[indices]).float().to(device)
                # images = images.to(device).to(torch.float32) / 127.5 - 1
                # labels = labels.to(device)

                loss = loss_fn(net=ddp, net_ref=net_ref, images=images, labels=labels)
                training_stats.report('Loss/loss', loss)
                loss.mul(loss_scaling).mean().backward()
                
        # Update weights.
        lr = learning_rate_schedule(ref_lr=optimizer_kwargs['lr'], cur_nimg=cur_nimg, total_kimg=total_kimg)
        ...
        # Sample Img
        if (sample_ticks is not None) and (done or cur_tick % sample_ticks == 0) and cur_tick > resume_tick and dist.get_rank() == 0:
            dist.print0('Exporting sample images...')
            images = [encoder.decode(generator_fn(ema, z, c)).cpu() for z, c in zip(grid_z, grid_c)]
            images = torch.cat(images).numpy()
            save_image_grid(images, os.path.join(run_dir, f'1_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
            ...
            images = torch.cat(images).numpy()
            save_image_grid(images, os.path.join(run_dir, f'2_step_{cur_tick:06d}_{cur_nimg//1000:07d}.png'), drange=[0,255], grid_size=grid_size)
        
            del images

        # Save full dump of the training state.
        if (state_dump_ticks is not None) and (done or cur_tick % state_dump_ticks == 0) and cur_tick > resume_tick and dist.get_rank() == 0:
            dist.print0(f'Save the latest dump at {cur_tick:06d} img...')
            torch.save(dict(net=net, net_ref=net_ref, ema=ema, optimizer_state=optimizer.state_dict()), os.path.join(run_dir, f'training-state-latest.pt'))
