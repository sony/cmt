    ...
    def __init__(self, backbone_type="VGG16_Latent_GN", backbone_in_channels=4,
        ...
        if pretrained_backbone_path is not None:
            if pretrained_backbone_path.endswith(".safetensors"):
                ...
                backbone = torch.load(pretrained_backbone_path, map_location='cpu')['model']
...
class ELatentLPIPS(nn.Module):
    def __init__(self, backbone_type="VGG16_Latent_GN", backbone_in_channels=4,
        ...
        backbone = torch.load(pretrained_latent_lpips_path, map_location='cpu', weights_only=False)['model']
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        # print(diffs[0].shape, self.linear_layers[0](diffs[0]).shape) # torch.Size([64, 64, 64, 64]) torch.Size([64, 1, 64, 64])
...
########### E Latent LPIPS Model ###########
class ELatentLPIPS2(nn.Module):
    def __init__(self, backbone_type="VGG16_Latent_GN", backbone_in_channels=4,
                 pretrained_latent_lpips_path=None, aug_type="bgc+cutout", aug_strength=1.0, 
                 use_dropout=True, eval_mode=True):
        super(ELatentLPIPS2, self).__init__()
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        res = [spatial_average(diffs[i].sum(dim=1,keepdim=True), keepdim=True) for i in range(5)]
