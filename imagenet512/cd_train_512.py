...
from training import cd_guid_training_loop_512 as training_loop
...
config_presets = {
   'edm2-img512-m':         dnnlib.EasyDict(channels=256, betas=(0.9, 0.99), decay=2000, dropout=0.50, dropres=16, mean=-0.8, std=1.6),
   'edm2-img512-m-safe':    dnnlib.EasyDict(channels=256, betas=(0.9, 0.99), decay=1000, dropout=0.00, dropres=None, mean=-0.8, std=1.6),
   'edm2-img512-s':         dnnlib.EasyDict(channels=192, betas=(0.9, 0.99), decay=1000, dropout=0.50, dropres=16, mean=-0.8, std=1.6),
   'edm2-img512-s-safe':    dnnlib.EasyDict(channels=192, betas=(0.9, 0.99), decay=1000, dropout=0.00, dropres=None, mean=-0.8, std=1.6),
   'edm2-img512-xxl':       dnnlib.EasyDict(channels=448, betas=(0.9, 0.99), decay=1000, dropout=0.50, dropres=16, mean=-0.8, std=1.6),
   'edm2-img512-xxl-safe':  dnnlib.EasyDict(channels=448, betas=(0.9, 0.99), decay=1000, dropout=0.00, dropres=None, mean=-0.8, std=1.6),
}
...
@click.option('--sch',           help='Scheduler Type', metavar='STR',                              type=str, default="Linear", show_default=True)
...
@click.option('--mean',          help='P_mean of Log Normal Distribution', metavar='FLOAT',         type=click.FloatRange(), default=-0.8, show_default=True)
@click.option('--std',           help='P_std of Log Normal Distribution', metavar='FLOAT',          type=click.FloatRange(), default=1.6, show_default=True)
...
@click.option('-solver',         help='One step solver tyep', metavar='STR',                        type=click.Choice(['euler', 'heun']), default='heun', show_default=True)
@click.option('-guid',           help='Guidance scale', metavar='FLOAT',                              type=click.FloatRange(), default=1.0, show_default=True)
@click.option('--loss_metric',   help='Loss mertric type', metavar='STR',                           type=str, default='Huber', show_default=True)
...
@click.option('--gnet',          help='Transfer learning from network pickle', metavar='PKL|URL',   type=str)
@click.option('--transfer2',     help='Transfer learning from network pickle', metavar='PKL|URL',   type=str)
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(P_mean=opts.mean, P_std=opts.std, q=opts.q, c=opts.c, k=opts.k, b=opts.b, adj=opts.mapping, wt=opts.wt, solver=opts.solver, guid=opts.guid, loss_metric=opts.loss_metric)
    ...
    c.loss_kwargs.class_name = 'training.loss.ECDGuidLoss'
    ...
    c.ref_network_kwargs.update(dropout=0, dropout_res=None, use_fp16=opts.fp16)
