...
config_presets = {
   'edm2-img512-s':         dnnlib.EasyDict(channels=192, betas=(0.9, 0.99), decay=2000, dropout=0.0, dropres=None, mean=-0.4, std=1.0),
   'edm2-img512-m':         dnnlib.EasyDict(channels=256, betas=(0.9, 0.99), decay=2000, dropout=0.0, dropres=None, mean=-0.4, std=1.0),
   'edm2-img512-xxl':       dnnlib.EasyDict(channels=448, betas=(0.9, 0.99), decay=2000, dropout=0.0, dropres=None, mean=-0.4, std=1.0),
}
...
@click.option('--encoder_pkl',   help='Load network pickles', metavar='PKL|URL|DIR',                type=str)
...
def main(**kwargs):
   ...
   # Initialize config dict.
    ...
    desc = f'{opts.preset:s}-gpus{dist.get_world_size():d}-{dtype_str:s}'
    ...
    c.encoder_pkl = opts.encoder_pkl
    evaluation_pkl(**c)
...
def evaluation_pkl(
    ...
    dist.print0(f'Loading encoder weights from "{encoder_pkl}"...')
    if dist.get_rank() != 0:
        torch.distributed.barrier() # rank 0 goes first
    with dnnlib.util.open_url(encoder_pkl, verbose=(dist.get_rank() == 0)) as f:
        data = pickle.load(f)
    if dist.get_rank() == 0:
    ...
    #     images = [encoder.decode(generator_fn(net, z, c)).cpu() for z, c in zip(grid_z, grid_c)]
    #     images = torch.cat(images).numpy()
    #     save_image_grid(images, os.path.join(run_dir, f'1_step.png'), drange=[0,255], grid_size=grid_size)
    
    #     few_step_fn = functools.partial(generator_fn, mid_t=mid_t)
    #     images = [encoder.decode(few_step_fn(net, z, c)).cpu() for z, c in zip(grid_z, grid_c)]
    #     images = torch.cat(images).numpy()
    #     save_image_grid(images, os.path.join(run_dir, f'2_step.png'), drange=[0,255], grid_size=grid_size)
    ...
    for _ in range(eval_repeat):
        one_step_results = metric_main.calc_metric(metric='fid50k_full',
                generator_fn=generator_fn, G=net, G_kwargs={}, encoder=encoder,
        ...
        few_step_results = metric_main.calc_metric(metric='two_step_fid50k_full', 
                generator_fn=few_step_fn, G=net, G_kwargs={}, encoder=encoder,
                dataset_kwargs=dataset_kwargs, num_gpus=dist.get_world_size(), rank=dist.get_rank(), device=device)
        if dist.get_rank() == 0:
