...
class MetricOptions:
    def __init__(self, generator_fn=None, G=None, G_kwargs={}, encoder={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None, progress=None, cache=True):
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=128, batch_gen=64, jit=False, **stats_kwargs):
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            indices = np.random.randint(0, 1000, size=batch_gen)
            c = np.eye(1000)[indices]
            c = torch.from_numpy(c).pin_memory().to(opts.device)
