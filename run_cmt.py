...
from training import cmt_training_loop as training_loop
...
@click.option('--sch',           help='Use scheduler or not', metavar='INT',                        type=int, default=0, show_default=True)
...
@click.option('--loss_metric',   help='Loss mertric type', metavar='STR',                           type=str, default='LPIPS', show_default=True)
...
def main(**kwargs):
    ...
    c.ref_network_kwargs = dnnlib.EasyDict()
    c.loss_kwargs = dnnlib.EasyDict(NFE=opts.nfe, loss_metric=opts.loss_metric)
    ...
    if opts.arch == 'ddpmpp':
        ...
        c.ref_network_kwargs.update(model_type='SongUNet', embedding_type='positional', encoder_type='standard', decoder_type='standard')
        c.ref_network_kwargs.update(channel_mult_noise=1, resample_filter=[1,1], model_channels=128, channel_mult=[2,2,2])
    elif opts.arch == 'ncsnpp':
        c.network_kwargs.update(model_type='SongUNet', embedding_type='fourier', encoder_type='residual', decoder_type='standard')
        c.network_kwargs.update(channel_mult_noise=2, resample_filter=[1,3,3,1], model_channels=128, channel_mult=[2,2,2])

        c.ref_network_kwargs.update(model_type='SongUNet', embedding_type='fourier', encoder_type='residual', decoder_type='standard')
        c.ref_network_kwargs.update(channel_mult_noise=2, resample_filter=[1,3,3,1], model_channels=128, channel_mult=[2,2,2])
    else:
        assert opts.arch == 'adm'
        c.network_kwargs.update(model_type='DhariwalUNet', model_channels=192, channel_mult=[1,2,3,4])
        c.ref_network_kwargs.update(model_type='DhariwalUNet', model_channels=192, channel_mult=[1,2,3,4])

    # Preconditioning & loss function.
    c.network_kwargs.class_name = 'training.networks.ECMPrecond'
    c.loss_kwargs.class_name = 'training.loss.DPMSolverLoss'

    c.ref_network_kwargs.class_name = 'training.networks.EDMPrecond'
    ...
    if opts.augment:
        ...
        c.ref_network_kwargs.augment_dim = 9
    c.network_kwargs.update(dropout=opts.dropout, use_fp16=opts.fp16)
    c.ref_network_kwargs.update(dropout=opts.dropout, use_fp16=opts.fp16)
    ...
    desc = f'{dataset_name:s}-{cond_str:s}-{opts.arch:s}-{opts.optim:s}-{opts.lr:f}-gpus{dist.get_world_size():d}-batch{c.batch_size:d}-{dtype_str:s}'
